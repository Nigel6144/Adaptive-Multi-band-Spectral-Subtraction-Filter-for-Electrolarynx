...
- `aud.wav` — Example noisy recording (place your input in the sd card formatted in Fat32 format).
- `out.wav` — Output generated by the script in the sd card
...
- Note:
  ...
  - Use AMD Vitis Classic not AMD Vitis Unified Platform to prevent compatibility issues.
  - File name length: by default FatFS in many Xilinx BSPs uses **8.3** filenames (max 8 chars name + 3 chars extension).        If you want long file names enable Long File Name (LFN) support in FatFS (`ffconf.h`) — e.g. set `FF_USE_LFN = 1` and        `FF_MAX_LFN = 255`.
...
- Enable FatFs in BSP Settings (xilffs) and rebuild BSP.
- Run the C Code
...
- Memory Pool DDR 128MB: Allocates a 128 MB memory pool in DDR for 
